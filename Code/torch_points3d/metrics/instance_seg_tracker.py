...
from torch_points3d.models.instance_seg.structures import PanopticResults, PanopticLabels
...
from torch_points3d.models.instance_seg.ply import read_ply, write_ply
...
class InstanceAPMeter:
    def __init__(self):
        self._pred_clusters = defaultdict(list)
        self._gt_clusters = defaultdict(lambda: defaultdict(list))
...
class PanopticTracker(SegmentationTracker):
    ...
    def block_merging(self, originids, origin_sub_ids, pre_sub_ins, all_pre_ins, max_instance, th_merge, outputs,
        ...
        if np.any(has_prediction):
            ...
            idx = np.argwhere(all_pre_ins[originids] != -1)
            idx2 = np.argwhere(all_pre_ins[originids] == -1)
    ...
    def block_merging_by_score(self, all_scores, all_clusters, new_scores, new_clusters, originids, origin_sub_ids):
        ...
        proposal_masks = torch.zeros(n_prop, self._test_area.y.shape[0])
